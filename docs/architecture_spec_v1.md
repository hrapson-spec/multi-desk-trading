...
### 14.9 Reliability-gate commitment (§12.2 point 3, v1.6)

**v1.6 calibration.** The v1.4 spec borrowed 28 days from AWS / Microsoft / SRE conventions designed for systems with real traffic and real capital at risk. On a synthetic-only research prototype the 28-day threshold was uncalibrated (it was named, not derived from a failure-model). The v1.6 revision drops it to **7 days** and compensates with richer instrumentation (the `soak/` runner's `ResourceMonitor` samples every 60 s and `IncidentDetector` enforces pre-registered numeric thresholds) so shorter runs still catch the meaningful failure modes. This is the v1.5-critique fix #1.

**Operator-side cost.** 7 days of wall-clock process uptime on a development machine. The `soak/` runner's checkpoint-and-resume design (v1.5-critique fix #2) tolerates OS reboots, `brew upgrade`, laptop sleep, and power events — provided the runner resumes cleanly from the checkpoint, elapsed time is cumulative and the clock does NOT reset. Only a checkpoint-corrupting failure (db_corruption incident) resets elapsed.
