# Multi-Desk Trading Architecture — Specification v1.16
...
| Version | Date | Change |
|---|---|---|
| v1.16 | 2026-04-22 | Desk-roster restructure adopting the external review in `docs/first_principles_redesign.md` (pasted-review version; supersedes the earlier in-progress first-principles content, never committed). **Oil shrinks 5 → 3 desks**: `storage_curve` kept; `supply` + `geopolitics` merge into `supply_disruption_news` (event-hurdle framing — activation probability × conditional effect size); `demand` + macro alpha merge into `oil_demand_nowcast` (mixed-frequency nowcast). Standalone `geopolitics` and `macro` desks REMOVED as alpha desks — `macro` becomes regime-conditioning state only via §10 `regime_classifier`. **Equity-VRP shrinks 2 → 1 shipped desk**: `dealer_inventory` + `hedging_demand` merge into `surface_positioning_feedback` (signed flow absent from `sim_equity_vrp/`; the pasted review's "if no signed flow, do not keep `customer_flow_pressure` standalone" rule applies). **Planned Phase 2 scale-out re-scoped**: `macro_regime` DROPPED (demoted to conditioning); `term_structure` DEFERRED pending review-consistency re-check; `earnings_calendar` kept (explicit in pasted review). **Target registry append**: `VIX_30D_FORWARD_3D_DELTA` added to `contracts/target_variables.py` — equity family emits signed 3-day delta, not level, so `Controller.decide` raw sum (§8.2) remains unit-consistent. `WTI_FRONT_1W_LOG_RETURN` already present (v1.15-era WIP). Pasted review's per-desk alpha targets (prompt-spread change, deferred-strip residual, next-session RV surprise, IV/skew delta) are INTERNAL/AUXILIARY labels only; controller-facing emission stays in one unified decision-space unit per family — frozen-controller constraint explicitly preserved. **Gates**: §12.2 item 2 Logic-gate threshold becomes ≥2/3 on the 3-desk oil roster (v1.11 ≥3/5 historical, rebaselined in C7). Strict invariants unchanged: `storage_curve` 3/3, Gate 3 all-seeds. D1 narrowed from "4 of 5 weak oil desks" to "2 of 3 weak oil desks"; D7 re-scoped to `surface_positioning_feedback` Gate 2 on merged channels. Full restructure tracked as D15 in `raid_log.md`. **Non-breaking**: no changes to §4, §6-§11; desk removal is a §12.1 decomposition change, pluggable under §5 Desk protocol. `controller/`, `contracts/v1.py`, `bus/`, `persistence/`, `eval/hot_swap.py`, `provenance/`, `scheduler/` unchanged. |
| v1.15 | 2026-04-18 | Post-v1.14 model/audit closure pack. (A) `HMMRegimeClassifier` moves from fixed `K=4` to adaptive-K Gaussian HMM selection (`K ∈ [2, 6]` by BIC), closing the live fixed-K weakness without changing the `RegimeLabel` contract. (B) `research_loop.handlers.regime_transition_handler` flips to the v0.3 validated path: grading-space Shapley + held-out margin validation before promotion; harmful desks do not retain positive candidate weight. (C) Feed reinstatement now prefers `historical_shapley_share`, then `latest_nonzero_weight_for_desk`, then the conservative direct seed-weight fallback, closing the over-broad D4 fallback claim. (D) Normalized grading-space Shapley ships and same-target attribution tests close D6 and D8. (E) Oil desk feature deepening lifts the Logic-gate aggregate from 5/10 to 6/10 seeds while preserving the strict invariants; D1 remains open but narrowed. (F) Phase 2 dealer models move to direct vol-delta heads on observed channels; Gate 1 is now positive for both desks, while Gate 2 remains the residual D7. |
| v1.0 | 2026-04-17 | Initial freeze. |
| v1.1 | 2026-04-17 | Review response. Addresses 16 items. Blockers: weight-promotion contradiction fixed (§8.3 vs §11), `target_variable` frozen registry (§4.6, `contracts/target_variables.py`), horizon semantics + event-slip policy (§4.7). Structural: CVaR cut from Phase 1, linear sizing committed (§8.2, §8.2a). Significant: weight-matrix dimensionality discipline (§8.5), provenance dirty-tree policy (§4.3), `data_ingestion_failure` trigger (§6.2), strengthened done-criterion (§12.2). Smaller: N/M pinned (§7.2), `regime_probabilities` on RegimeLabel (§4.3), routing rules as postconditions (§6.4), Macro vs classifier clarified (§10.1), `attribution_lodo` grain fixed + `decisions` table added (§3.2). New flagged risks: §14.6 budget realism, §14.7 Phase 2 readiness, §14.8 cold-start policy. |
...
| v1.14 | 2026-04-18 | D9 closure — Gate 3 runtime hot-swap harness. Prior to v1.14 the integration-level Gate 3 callsites passed `run_controller_fn=lambda: True, run_controller_with_stub_fn=lambda: True` to `GateRunner.run`, making `gate3_hot_swap.passed == True` a tautology; the gate shell at `eval/gates.py:200-237` was always correct — the fault was at every integration-level callsite. **Gate 3 evidence prior to v1.14 is attribute-conformance only; runtime hot-swap is guaranteed from v1.14 onward.** New `eval/hot_swap.py::build_hot_swap_callables()` factory: seeds `controller_params` + `signal_weights`, runs `Controller.decide()` with the real desk, then a shallow-copied variant with a `StubDesk` swap, and asserts (a) Decision validity, (b) `combined_signal_stub == combined_signal_real − weight_real × point_estimate_real` (with explicit branches for stale-real and stale-stub interactions, per B-3b/d), (c) honest `contributing_ids` membership (desk's forecast_id is in real IDs iff non-stale; never in stub IDs). Local `_DeskLike` `@runtime_checkable` Protocol preserves the §8.4 portability contract (no `desks.*` import from `eval/`). `gate_hot_swap` metrics schema extended with `failure_mode` enum: `"passed" | "controller_exception" | "assertion_failure"` — the AssertionError-specific branch distinguishes "closure's assertion failed" from "closure raised an uncaught exception." **Load-bearing side-effect (B-4 Controller fix):** the closed-loop exercise surfaced a real bug at `controller/decision.py:96-104` — a retired desk's (weight=0) forecast_id was appended to `contributing_ids` after the staleness check, leaving retired desks visible in Decision output while contributing 0 to `combined_signal`. Fix: `if w == 0.0: continue` guard before the append. Regression test at `tests/test_controller_retire_exclusion.py`. 7 integration callsites migrated (`tests/test_phase_a_clean_observations.py`, `tests/test_phase_b_controlled_leakage.py`, `tests/test_phase_c_realistic_contamination.py`, `tests/test_logic_gate_multi_scenario.py`, `tests/test_storage_curve_gates.py`, `tests/test_dealer_inventory_gates.py`, `tests/test_hedging_demand_gates.py`); the 4 shell-unit tests in `tests/test_gates.py` keep their `lambda: True` literals because they legitimately test gate_hot_swap's pass-through contract, not the integration path. New same-target two-desk case at `tests/test_hot_swap_two_desk.py` pins the D8 production scenario: swapping one of two same-target desks (dealer_inventory ⊕ hedging_demand, both targeting `VIX_30D_FORWARD`) preserves the other desk's contribution, and verifies the B-3a shallow-copy invariant (stub-variant cannot mutate the caller's `recent_forecasts_other` dict). Test-DB teardown audit: every migrated callsite uses pytest `tmp_path` or `:memory:`, guaranteeing per-test isolation. Full suite: 397 passed + 1 skipped. D9 closed with scope caveat — coverage limited to the 7 migrated callsites; any future Gate 3 test that wires `lambda: True` is expected to use the helper instead. |
...
2. **Logic gate** — simulated-time replay. The architecture is exercised end-to-end across `N_scenarios ≥ 10` independent seeds × regime sequences; each scenario replays ≥ 4 weeks of simulated events through the full loop (desks → Controller → grading → attribution → research-loop weight promotion). **Strict invariants (v1.11 load-bearing, hold on 10/10 seeds):** storage_curve passes all three gates + Gate 3 (hot-swap) passes for all 5 desks. **Capability claim (v1.11 — Phase A ridge-on-4-features debit, `capability_debits.md` D1):** per-scenario aggregate ≥ 3/5 desks pass Gate 1 and Gate 2; across scenarios the per-scenario aggregate threshold holds on ≥ 5/10 seeds. Non-storage-curve desks' skill improvement is a §7.3 Phase 2 escalation item, not a Phase 1 blocker. Simulated-time testing isolates **logic correctness** and completes in minutes per run. Test: `tests/test_logic_gate_multi_scenario.py`. **v1.16 rebase note**: the 5-desk oil roster is superseded by the 3-desk roster (`storage_curve`, `supply_disruption_news`, `oil_demand_nowcast`) per `docs/first_principles_redesign.md`. The live aggregate rule becomes **≥ 2/3 desks pass Gate 1 + Gate 2** (same test file, rebaselined in C7 of the restructure plan); strict invariants (`storage_curve` 3/3 + Gate 3 all-seeds) unchanged. The v1.15 "6/10 seeds" achievement is historical and does not transfer to the restructured roster — the v1.16 baseline is re-measured in Phase 2 scale-out work.
...
2. **Rolling failure-rate rules (Layer 2).** A new periodic handler `feed_reliability_review_handler` reads `feed_incidents` over a lookback (default 30 days) and applies pre-registered thresholds:
    ...
    - **Reinstatement (v1.15)**: a feed with no failures for `recovery_days` (default 14) and no currently-open incident triggers reinstatement. Primary path: `historical_shapley_share(conn, desk_name, lookback_days=90, now_utc)` computes the desk's mean share of total \|Shapley\| across recent reviews (bounded [0, 1]); if non-None and positive, the value is used as the reinstatement weight via `remediation.reinstate_desk_direct`. Secondary path when recent attribution rows are absent but the desk has a prior live weight: `latest_nonzero_weight_for_desk(...)`. Final fallback when neither attribution nor weight history exists: `reinstate_desk_direct(weight=reinstate_weight)` (default 0.1) — conservative cold-start weight letting the desk re-earn through the next review. All paths write tagged `"reinstate:feed_recovered:<feed>"`. Single-desk targeted — does not disturb other desks' weights.
...
**v1.16 restructure note.** The four-desk expansion is superseded by the adopted external review in `docs/first_principles_redesign.md`. Live Phase 2 equity-VRP roster: `surface_positioning_feedback` (merged `dealer_inventory` + `hedging_demand`) + `earnings_calendar` (planned, pasted review explicitly keeps). `macro_regime` DROPPED — demoted to regime-conditioning state via §10 `regime_classifier`. `term_structure` DEFERRED pending review-consistency re-check (pasted review silent). The §14.7 "four remaining" claim is historical. Current Phase 2 done-criterion lives in `docs/pm/master_plan.md` (scaled: 2 equity desks pass Gate 3; ≥ 1/2 pass Gates 1+2 aggregate).
...
| Post-phases-abc follow-ups: (1) HMM regime classifier — adaptive-K Gaussian HMM via hmmlearn, replacing the Phase A ground-truth pass-through; causal inference on observations[:i+1]; seed-deterministic fingerprint; default `K` selected by BIC in `[2, 6]`. (2) §6.4 LLM routing postcondition gate — `LLMArtefact` contract type + `commit_gate` validator rejecting local-tier outputs in {spec_edit, hypothesis_proposal, cross_desk_synthesis}; citations override auto-reclassifies as cross_desk_synthesis at ≥ 2 distinct desks. | Tags `hmm-classifier-v0.2`, `llm-routing-v0.1` (v1.5 revision; v1.15 adaptive-K upgrade) |
...
| D9 closure — Gate 3 runtime hot-swap harness (v1.14). `eval/hot_swap.py::build_hot_swap_callables()` factory replaces `run_controller_fn=lambda: True` at 7 integration-level callsites with a real `Controller.decide()` + `StubDesk` swap. Shell signature stays `Callable[[], bool]`; closure assertions verify Decision validity, `combined_signal` delta (with branches for stale-real / stale-stub), and honest `contributing_ids` membership. `gate_hot_swap.metrics.failure_mode` enum field added (`"passed" | "controller_exception" | "assertion_failure"`). Load-bearing Controller fix shipped alongside: `controller/decision.py:96-104` now skips `contributing_ids.append` when `weight == 0.0`, excluding retired desks from Decision output (B-4 regression surfaced by the closed-loop exercise). Same-target two-desk case (`tests/test_hot_swap_two_desk.py`) pins the D8 production scenario. Full suite: 397 passed + 1 skipped. D9 closed with scope caveat — 7 migrated callsites carry runtime evidence; `lambda: True` in shell-unit tests (`tests/test_gates.py`) is legitimate pass-through testing. | Tags `gate3-runtime-harness-v1.14` |
