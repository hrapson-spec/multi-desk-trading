...
2. **Rolling failure-rate rules (Layer 2).** A new periodic handler `feed_reliability_review_handler` reads `feed_incidents` over a lookback (default 30 days) and applies pre-registered thresholds:
    ...
    - **Reinstatement**: a feed with no failures for `recovery_days` (default 14) and no currently-open incident triggers reinstatement. Primary path: Shapley-proportional promotion (§8.3). Fallback when Shapley has no recent rows for the retired desk: `remediation.reinstate_desk_direct(weight=0.1)` writes a conservative non-zero weight tagged `"reinstate:feed_recovered:<feed>"`, letting the desk re-earn weight through the next Shapley review.
...
### 14.9 Reliability-gate commitment (§12.2 point 3, v1.8)

**v1.8 calibration.** The v1.4 spec borrowed 28 days from AWS / Microsoft / SRE conventions designed for systems with real traffic and real capital at risk. v1.6 dropped to 7 days, compensated by richer instrumentation. Under further scrutiny — for a synthetic-only research prototype the 7-day duration is still partially gut-feel. The meaningful failure modes the gate catches are:

| Failure mode | Actual exposure time required |
|---|---|
| Memory leaks | Hours (fast) to 24–48h (slow, tens of MB/day). |
| File-descriptor leaks | Minutes to hours. |
| Disk growth against the 5 GB threshold | Never reached at the drip-feed rate — the threshold is for catastrophic runaway, not normal growth. |
| Scheduler / DB / bus exceptions | Random rate; bounded by scheduler cadence, not duration. |
| Daily-cycle bugs (log rotation, tmp cleanup, periodic cron) | ≥ 24 hours. |

The v1.8 revision drops the duration to **48 hours** — the defensible floor that still exposes daily-cycle bugs while removing the "longer than a day just feels serious" slack. Operational-maturity signal is preserved (SRE convention of an overnight-plus-a-day endurance test); critical-path bottleneck is reduced. Instrumentation is unchanged: the 48-hour run still catches ≥ 90% of what 7 days would catch at ≈ 30% of the wall-clock cost.

**Operator-side cost.** 48 hours of wall-clock process uptime on a development machine. The `soak/` runner's checkpoint-and-resume design (v1.5-critique fix #2) tolerates OS reboots, `brew upgrade`, laptop sleep, and power events — provided the runner resumes cleanly from the checkpoint, elapsed time is cumulative and the clock does NOT reset. Only a checkpoint-corrupting failure (db_corruption incident) resets elapsed.
