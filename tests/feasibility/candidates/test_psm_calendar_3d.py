"""Tests for the PSM calendar audit-only candidate."""
...
import feasibility.candidates.psm_calendar_3d.classical as mod
...
def _make_psm_events(n: int = 12) -> pd.DataFrame:
    idx = pd.date_range("2020-01-31 15:05", periods=n, freq="ME", tz="UTC")
...
def test_build_psm_calendar_features_uses_declared_columns() -> None:
    events = _make_psm_events()

    result = build_psm_calendar_features(events)

    assert tuple(result.columns) == PSM_FEATURE_COLUMNS
    assert len(result) == len(events)
    assert np.isfinite(result.to_numpy()).all()
    assert set(result["quarter_end_indicator"].unique()).issubset({0.0, 1.0})


def test_build_psm_calendar_features_rejects_missing_columns() -> None:
    events = _make_psm_events().drop(columns=["release_date"])

    with pytest.raises(ValueError, match="missing required columns"):
        build_psm_calendar_features(events)


def test_build_psm_calendar_features_rejects_non_datetime_index() -> None:
    events = _make_psm_events().reset_index(drop=True)
...
def test_module_does_not_export_desk_class() -> None:
    ...
    assert not isinstance(PSMCalendarLogisticModel(), DeskProtocol)


def test_module_path_is_under_feasibility_not_desks() -> None:
    loaded = importlib.import_module("feasibility.candidates.psm_calendar_3d.classical")
