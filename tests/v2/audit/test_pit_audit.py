...
def _ingest(w, source, release_ts):
    w.write_vintage(
        source=source,
        series="main",
...
def test_audit_reports_per_source_stats(store_and_calendars):
    ...
    _ingest(w, "eia_wpsr", datetime(2026, 1, 14, 15, 30, tzinfo=UTC))
    _ingest(w, "eia_wpsr", datetime(2026, 1, 21, 15, 30, tzinfo=UTC))
    _ingest(w, "cftc_cot", datetime(2026, 1, 16, 20, 30, tzinfo=UTC))
    auditor = PITAuditor(root, m, calendars)
    report = auditor.audit()
    assert report.is_clean, report.issues
    assert report.sources["eia_wpsr"].vintage_count == 2
    assert report.sources["cftc_cot"].vintage_count == 1
    assert report.sources["eia_wpsr"].calendar_present is True
...
def test_audit_training_window_earliest(store_and_calendars):
    root, w, m, calendars = store_and_calendars
    _ingest(w, "eia_wpsr", datetime(2025, 1, 14, 15, 30, tzinfo=UTC))
    _ingest(w, "eia_wpsr", datetime(2026, 1, 14, 15, 30, tzinfo=UTC))
...
def test_audit_detects_checksum_failure(store_and_calendars):
    ...
    report = auditor.audit()
    assert not report.is_clean
    assert report.sources["eia_wpsr"].checksum_failures == 1
