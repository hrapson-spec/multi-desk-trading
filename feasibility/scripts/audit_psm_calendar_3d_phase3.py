"""Phase 3 audit orchestration for PSM calendar -> WTI 3d sign.

Pre-registered at feasibility/preregs/2026-04-29-psm_calendar_wti_3d.yaml.
...
from feasibility.candidates.psm_calendar_3d.classical import (
    PSM_FEATURE_COLUMNS,
    PSMCalendarLogisticModel,
    build_psm_calendar_features,
)
...
RESIDUALS_CSV = REPO_ROOT / "feasibility" / "outputs" / "psm_calendar_3d_residuals.csv"
PHASE3_MANIFEST = (
    REPO_ROOT / "feasibility" / "outputs" / "tractability_v1_3d_phase3_audit_psm_calendar.json"
)
PHASE3_REPORT = (
    REPO_ROOT / "feasibility" / "reports" / "terminal_2026-04-29_phase3_audit_psm_calendar.md"
)
...
def load_psm_events(pit_root: Path) -> pd.DataFrame:
    """Load PSM calendar PIT rows indexed by release usable timestamp."""
    ...
    try:
        rows = conn.execute(
            ...
            FROM pit_manifest
            WHERE source = 'eia_psm'
              AND dataset = 'psm_calendar'
    ...
    for usable_after_ts, parquet_path in rows:
        ...
        if frame.empty or not {"issue_label", "release_date"} <= set(frame.columns):
            continue
        records.append(
            (
                _utc_timestamp(usable_after_ts),
                str(frame["issue_label"].iloc[0]),
                str(frame["release_date"].iloc[0]),
            )
        )

    if not records:
        raise ValueError("no usable PSM calendar PIT rows found")

    return (
        pd.DataFrame(records, columns=["release_ts", "issue_label", "release_date"])
        .drop_duplicates(subset=["release_ts"], keep="first")
...
def build_event_features_and_labels(
    ...
    anchored_psm_features: pd.DataFrame,
    prices: pd.Series,
) -> tuple[np.ndarray, np.ndarray, pd.DatetimeIndex]:
    """Build X/y/decision_ts rows for kept PSM target observations."""
    feat_rows: list[list[float]] = []
    label_rows: list[int] = []
    ts_rows: list[pd.Timestamp] = []

    for obs in kept_obs:
        if obs.decision_ts not in anchored_psm_features.index:
            continue
        lagged = _compute_wti_5d_lagged_return(obs.decision_ts, prices)
        if lagged is None:
            continue
        release_features = anchored_psm_features.loc[obs.decision_ts]
        feat_rows.append(
            [
                *[float(release_features[col]) for col in PSM_FEATURE_COLUMNS],
                lagged,
            ]
        )
    ...
    if not feat_rows:
        return (
            np.empty((0, len(PSM_FEATURE_COLUMNS) + 1), dtype=float),
...
def walk_forward_audit(
    ...
    *,
    ...
    for year, month in months:
        ...
        model = PSMCalendarLogisticModel()
...
def write_report(manifest: dict, metrics: WalkForwardAudit, report_path: Path) -> None:
    """Write the PSM Phase 3 audit verdict report."""
    ...
    lines = [
        ...
        "",
        "> Caveat: this audit uses the v1.0 PIT release calendar only. It does not test "
        "EIA-914/PSM production-surprise values, which require a v1.1 table parser.",
        "",
        "---",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    psm_events = load_family_decision_events(args.pit_root, EIA_PSM_FAMILY)
    ...
    obs = build_target_observations([psm_events], prices, horizon_days=HORIZON_DAYS)
    ...
    event_frame = load_psm_events(args.pit_root)
    release_features = build_psm_calendar_features(event_frame)
    anchored_features = anchor_release_features_to_target_prices(
        release_features,
        psm_events.decision_ts,
        prices,
    )
