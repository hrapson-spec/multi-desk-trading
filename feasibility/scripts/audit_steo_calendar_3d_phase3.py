"""Phase 3 audit orchestration for STEO calendar -> WTI 3d sign.

Pre-registered at feasibility/preregs/2026-04-29-steo_calendar_wti_3d.yaml.
...
RESIDUALS_CSV = REPO_ROOT / "feasibility" / "outputs" / "steo_calendar_3d_residuals.csv"
PHASE3_MANIFEST = (
    REPO_ROOT / "feasibility" / "outputs" / "tractability_v1_3d_phase3_audit_steo_calendar.json"
)
PHASE3_REPORT = (
    REPO_ROOT / "feasibility" / "reports" / "terminal_2026-04-29_phase3_audit_steo_calendar.md"
)
...
def load_steo_events(pit_root: Path) -> pd.DataFrame:
    """Load STEO calendar PIT rows indexed by release usable timestamp."""
    ...
    try:
        rows = conn.execute(
            ...
            FROM pit_manifest
            WHERE source = 'eia_steo'
              AND dataset = 'steo_calendar'
    ...
    if not records:
        raise ValueError("no usable STEO calendar PIT rows found")
...
def build_event_features_and_labels(
    ...
    anchored_steo_features: pd.DataFrame,
    prices: pd.Series,
) -> tuple[np.ndarray, np.ndarray, pd.DatetimeIndex]:
    """Build X/y/decision_ts rows for kept STEO target observations."""
    feat_rows: list[list[float]] = []
    label_rows: list[int] = []
    ts_rows: list[pd.Timestamp] = []

    for obs in kept_obs:
        if obs.decision_ts not in anchored_steo_features.index:
            continue
        lagged = _compute_wti_5d_lagged_return(obs.decision_ts, prices)
        if lagged is None:
            continue
        release_features = anchored_steo_features.loc[obs.decision_ts]
        feat_rows.append(
            [
                *[float(release_features[col]) for col in STEO_FEATURE_COLUMNS],
                lagged,
            ]
        )
    ...
    if not feat_rows:
        return (
            np.empty((0, len(STEO_FEATURE_COLUMNS) + 1), dtype=float),
...
def write_report(manifest: dict, metrics: WalkForwardAudit, report_path: Path) -> None:
    """Write the STEO Phase 3 audit verdict report."""
    ...
    lines = [
        ...
        "STEO forecast table values, which require a v1.1 table parser.",
        "",
        "---",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    steo_events = load_family_decision_events(args.pit_root, STEO_FAMILY)
    ...
    obs = build_target_observations([steo_events], prices, horizon_days=HORIZON_DAYS)
    ...
    event_frame = load_steo_events(args.pit_root)
    release_features = build_steo_calendar_features(event_frame)
    anchored_features = anchor_release_features_to_target_prices(
        release_features,
        steo_events.decision_ts,
        prices,
    )
