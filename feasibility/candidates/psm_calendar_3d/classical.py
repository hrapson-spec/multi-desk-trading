"""Audit-only PSM calendar-pulse model for WTI 3-day return sign.
...
PSM_FEATURE_COLUMNS: tuple[str, ...] = (
    "issue_month_sin",
    "issue_month_cos",
    "quarter_end_indicator",
    "release_gap_days_scaled",
)


def build_psm_calendar_features(events: pd.DataFrame) -> pd.DataFrame:
    """Build release-calendar features from PSM calendar PIT rows.

    The current PSM ingester emits calendar metadata only. These features are
    therefore a narrow calendar-pulse baseline, not an EIA-914 production-value
    surprise model.
    """
    required = {"issue_label", "release_date"}
    missing = sorted(required - set(events.columns))
    if missing:
        raise ValueError(f"PSM events missing required columns: {missing}")

    if not isinstance(events.index, pd.DatetimeIndex):
        raise ValueError("PSM events must be indexed by release timestamp")
    ...
    features["quarter_end_indicator"] = release_dates.dt.month.isin([3, 6, 9, 12]).astype(float)
    features["release_gap_days_scaled"] = (release_gap_days.astype(float) - 30.5) / 5.0
    return features.loc[:, PSM_FEATURE_COLUMNS]


class PSMCalendarLogisticModel:
    """Logistic regression model: PSM calendar features -> WTI 3d sign."""
